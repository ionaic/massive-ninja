...
float nbhd_dist(ivec2 res_ij, ivec2 ex_ij, int nbhd_size) {
    ...
    ivec2 c_res = clamp(res_ij, shift, textureSize(res, 0));
    ivec2 c_ex = clamp(ex_ij, shift, textureSize(res, 0));
    ...
    for (int i = 0; i < nbhd_size; i++) {
        for (int j = 0; j < nbhd_size; j++) {
            dist += pow(texture(ex, c_ex) - texture(ex, texelFetch(res, c_res)), 2);
        }
    }
 
    // return summed squared euclidean distance for the neighborhood
