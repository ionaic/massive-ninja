...
#define k 4 // the number of values to return in the set

out ivec4 colorOut; // output color at this pixel
...
float nbhd_dist(ivec2 res_ij, ivec2 ex_ij, int k) {
    ...
    ivec2 c_res = clamp(res_ij, shift, textureSize(res, 0) - 3);
    ivec2 c_ex = clamp(ex_ij, shift, textureSize(ex, 0));
